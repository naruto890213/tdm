...
struct threadpool* threadpool_init(int thread_num, int queue_max_num, void* (*callback_function)(void *arg))
{
    ...
    do
    {
        ...
        pool->head = NULL;
        pool->tail = NULL;
		pool->callback_function = callback_function;
...
int threadpool_add_job(struct threadpool* pool, void *arg, int len)
{
    assert(pool != NULL);
    assert(arg != NULL);
	...
	{
		LogMsg(MSG_MONITOR, "This come from %s:%d, the queue_cur_num is %d\n", __func__, __LINE__, pool->queue_cur_num);
	}

    while ((pool->queue_cur_num == pool->queue_max_num) && !(pool->queue_close || pool->pool_close))
    {
		LogMsg(MSG_MONITOR, "This come from %s:%d, the queue_cur_num is queue_max_num is %d\n", __func__, __LINE__, pool->queue_max_num);
        pthread_cond_wait(&(pool->queue_not_full), &(pool->mutex));   //队列满的时候就等待
    }

    if (pool->queue_close || pool->pool_close)    //队列关闭或者线程池关闭就退出
    {
		LogMsg(MSG_MONITOR, "This come from %s:%d, pool->queue_close || pool->pool_close\n", __func__, __LINE__);
    ...
    if (NULL == pjob)
    {
		LogMsg(MSG_MONITOR, "This come from %s:%d malloc err\n", __func__, __LINE__);
        pthread_mutex_unlock(&(pool->mutex));
        return -1;
    }
...
void *threadpool_function(void* arg)
{
    ...
    while (1)  //死循环
    {
        ...
        (*(pool->callback_function))(pjob->Buff);   //线程真正要做的工作，回调函数的调用
        free(pjob);
        pjob = NULL;
    }
}
